...
export const animDb = {
  fund: {
    ...
    theory: [
      {
        title: "Abstraksi Bahasa Mesin",
        desc: "Bahasa mesin adalah level instruksi paling dasar yang dieksekusi langsung oleh prosesor (CPU). Tidak ada konsep variabel atau <i>if-else</i>; yang ada hanyalah pergerakan data antar <b>Register</b> dan <b>Memori Fisik</b>. C++ mengabstraksi hal ini agar kita fokus pada logika, namun pada akhirnya kode C++ tersebut akan di-<i>compile</i> menjadi jutaan deret instruksi bahasa mesin."
      },
      {
        title: "Anatomi Instruksi Assembly",
        desc: "Sebuah baris perintah pergerakan atau komputasi disebut sebagai <b>Instruksi</b>. Format umum penulisan instruksi bahasa rakitan (Assembly) adalah: <br><br><code>[Mnemonic] [Operand Target], [Operand Sumber]</code><br><br>Misal: <code>mov eax, 5</code>. <code>mov</code> adalah Mnemonic, sedangkan <code>eax</code> dan <code>5</code> adalah Operands."
      },
      {
        title: "Mnemonic & Opcode",
        desc: "<b>Opcode</b> (<i>Operation Code</i>) adalah kode heksadesimal murni yang dibaca CPU (contoh: <code>B8</code>). Karena manusia kesulitan membaca deret angka ini, digunakanlah <b>Mnemonic</b>. Mnemonic adalah terjemahan bahasa Inggris dari Opcode tersebut (contoh kode <code>B8</code> direpresentasikan sebagai mnemonic <code>MOV</code>)."
      },
      {
        title: "Blok Penulisan (Sections)",
        desc: "Struktur penulisan Assembly dipilah menjadi beberapa blok utama yang disebut <i>section</i>:<br>• <b>.data</b>: Menyimpan memori/variabel global statis (misal teks/string) yang sudah memiliki nilai awal.<br>• <b>.bss</b>: Menyimpan memori kosong yang belum diinisialisasi.<br>• <b>.text</b>: Area <i>read-only</i> yang berisi logika <i>executable</i> kode/instruksi program kita."
      }
    ],
    cppCode: `<span class="text-slate-400">// Di C++, operasi matematika dan
// manajemen memori ditangani oleh compiler.</span>
...
int main() {
    ...
    asmCode: {
      x64: `<span class="text-slate-400">; Assembly x64 mengharuskan kita memindahkan data
; secara manual dengan register modern (64-bit).</span>

main:
    <span class="text-slate-400">; Prologue (Memori Setup)</span>
    push rbp
    mov rbp, rsp
    sub rsp, 16            <span class="text-slate-400">; Alokasi stack 16 bytes</span>
    
    <span class="text-slate-400">; Variabel lokal (a=5, b=10)</span>
    mov dword [rbp-4], 5
    mov dword [rbp-8], 10
    
    <span class="text-slate-400">; Operasi Matematika (c = a + b)</span>
    mov eax, dword [rbp-4] <span class="text-slate-400">; eax = a</span>
    add eax, dword [rbp-8] <span class="text-slate-400">; eax = eax + b</span>
    mov dword [rbp-12], eax<span class="text-slate-400">; c = eax</span>
    
    <span class="text-slate-400">; return c; (Nilai diletakkan di EAX)</span>
    mov eax, dword [rbp-12]
    
    <span class="text-slate-400">; Epilogue (Memori Cleanup)</span>
    add rsp, 16
    pop rbp
    ret`,
      x86: `<span class="text-slate-400">; Assembly x86 (32-bit) menggunakan register
; klasik (eax, ebp, esp) untuk manajemen memori.</span>

main:
    <span class="text-slate-400">; Prologue (Memori Setup)</span>
    push ebp
    mov ebp, esp
    sub esp, 16            <span class="text-slate-400">; Alokasi stack 16 bytes</span>
    ...
    <span class="text-slate-400">; Operasi Matematika (c = a + b)</span>
    mov eax, dword [ebp-4] <span class="text-slate-400">; eax = a</span>
    add eax, dword [ebp-8] <span class="text-slate-400">; eax = eax + b</span>
    mov dword [ebp-12], eax<span class="text-slate-400">; c = eax</span>
